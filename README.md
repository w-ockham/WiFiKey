...
### CI-Vコントロールのリモート化
IC-705をリモートからCI-Vを使ってコントロールできます。まずサーバーで`Bluetooth CI-V`を`Enable`にします。IC-705の設定画面を開くとサーバー名のデバイスが表示されますのでペアリングを行えば接続完了です。次にクライアントも`Bluetooth CI-V`を`Enable`にしシリアルポートの速度を設定します。クライアントを接続したPCにCOMポートが現れますので、このポートにCI-Vコントロールソフトを接続することでサーバーに接続したIC-705を制御できます。
